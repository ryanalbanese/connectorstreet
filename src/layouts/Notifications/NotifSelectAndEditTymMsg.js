...
@connectWithNavigationIsFocused(
  state => ({
    ...
    pushIdsLocal: state.pushIdsLocals
  }),
  dispatch => ({
...
export default class SelectAndEditMsg extends Component {
  ...
  componentWillMount() {
    ...
    const { userData, getUserCustomMessages, actionGetCustomMessages,savedMessages,actionGetUsersPushIds, resetMakeIntroductionData, navigation } = this.props

    actionGetCustomMessages(userData.userModel.user_uid)
    let mateUserModel = ''

    mateUserModel = navigation.state.params.detailsData.userBy
    actionGetUsersPushIds([mateUserModel.user_uid])

  }

  componentWillReceiveProps(nextProps, state) {

    const { userData, navigation, makeIntroductionData, actionGetUsersSettings, setCustomMessages, updateCustomMessages, actionUpdateCustomMessages,getUserCustomMessages, actionGetCustomMessages, actionAddCustomMessages, actionAddNotification, resetMakeIntroductionData } = this.props
  ...
  confirm = () => {
    ...
    navigation.navigate('MakeIntroSent', {refresh: this.refresh})
  }
  ...
  sendData = () => {
    ...
    if (mateUserModel.user_uid) {
        ...
        let notificationId = mateUserModel.user_uid + '_' + userData.userModel.user_uid + '_' + moment().unix()
                  ...
                  <View style={styles.textsWrapper}>
                    ...
                    <View style={styles.messagesListWrapperLast}>
                      ...
                      <DefaultMessagesList
                       ...
                       headerText={
                        'Select a message below or create your own. '
                       }
